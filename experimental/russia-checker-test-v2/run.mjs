...
const CORE_NODES = String(process.env.RUSSIA_TEST_CORE_NODES || "ru2.node.check-host.net,ru3.node.check-host.net")
  .split(/[\s,;]+/).map(v => v.trim()).filter(Boolean);
const CORE_QUORUM = Math.max(1, Number(process.env.RUSSIA_TEST_CORE_QUORUM) || CORE_NODES.length);
...
function parseNodeResult(raw, node, transport) {
  ...
  if (raw && typeof raw === "object" && Number.isFinite(Number(raw.time))) {
    return { node, state: "reachable", raw, latencyMs: Number(raw.time) * 1000 };
  }
  if (/connection refused|unreachable|timed out|timeout|no route/i.test(`${text} ${jsonText}`)) {
...
function decide(nodes, transport) {
  if (nodes.length < CORE_QUORUM) return { verdict: "UNKNOWN", confidence: "insufficient-node-results" };
  if (transport === "tcp") {
    const reachable = nodes.filter(n => n.state === "reachable").length;
    if (reachable >= CORE_QUORUM) return { verdict: "PASS", confidence: "tcp-confirmed" };
    return { verdict: "FAIL", confidence: "tcp-confirmed" };
  }
  if (nodes.some(n => n.state === "refused")) return { verdict: "FAIL", confidence: "udp-explicitly-refused" };
  const usable = nodes.filter(n => n.state === "udp-filtered" || n.state === "reachable").length;
  if (usable >= CORE_QUORUM) return { verdict: "PASS-UDP-NOT-REFUSED", confidence: "udp-transport-not-refused" };
...
async function pollCheckHostMeasurement(measurement) {
  ...
  while (Date.now() - started <= MAX_POLL_MS) {
    ...
    if (decision.verdict === "PASS" || decision.verdict === "FAIL" || decision.verdict === "PASS-UDP-NOT-REFUSED" || allFinal) {
...
async function checkEndpoint(endpoint) {
  if (DRY_RUN) return { verdict: "DRY-RUN", confidence: "not-tested", requestId: "", nodes: CORE_NODES.map(node => ({ node, state: "dry-run", latencyMs: 0 })) };
...
function expandPassingLinks(items, endpointRows) {
  ...
  for (const item of items) {
    ...
    if (["PASS", "PASS-UDP-NOT-REFUSED", "DRY-RUN"].includes(verdict)) links.push(link);
  }
...
function runSelfTest() {
  ...
  if (decide([{state:"udp-filtered"},{state:"udp-filtered"}], "udp").verdict !== "PASS-UDP-NOT-REFUSED") throw new Error("UDP decision self-test failed");
...
async function main() {
  ...
  const work = SCOPE === 'lte' ? [['lte', lte]] : SCOPE === 'regular' ? [['regular', regular]] : [['regular', regular], ['lte', lte]];
  ...
  const md = [];
  md.push("# Russia checker experiment v2", "", `Generated: ${new Date().toISOString()}`, `Scope: ${SCOPE}`, `Core Check-Host gate: ${CORE_NODES.join(', ')}`, `Core quorum: ${CORE_QUORUM}/${CORE_NODES.length}`, "");
  ...
  for (const [label, report] of Object.entries(reports)) {
    md.push(`## ${label.toUpperCase()}`, "", `Candidates: **${report.candidates}**`, `Protocols: ${Object.entries(report.protocols).map(([k,v]) => `**${k}=${v}**`).join(', ') || 'none'}`, `Unique endpoints: **${report.uniqueEndpoints}**`, `Verdicts: ${Object.entries(report.endpointVerdicts).map(([k,v]) => `**${k}=${v}**`).join(', ') || 'none'}`, `HAPP-ready links: **${lists[label].length}**`, `Copy: [locations-${label}.txt](./locations-${label}.txt)`, "");
  }
  md.push("## Hysteria / UDP", "", "Hysteria/Hysteria2/TUIC are detected from the URI scheme and checked with Check-Host UDP, never TCP. The original link is copied to the output unchanged; no Hysteria link is converted to VLESS.", "", "`PASS-UDP-NOT-REFUSED` means the Russian gate did not receive an explicit UDP refusal. Check-Host itself documents the silent UDP state as `Open or filtered`, so this is a transport screening result, not proof of a successful Hysteria/QUIC handshake. Those links are deliberately left in the HAPP test list for manual verification.", "");
  ...
  md.push("## Files for your manual test", "", "- `locations-lte.txt` — all passing LTE links, with their original URI schemes preserved.", "- `lte-hysteria-all.txt` — every Hysteria/Hysteria2/TUIC LTE candidate before filtering.", "- `lte-hysteria-passing.txt` — Hysteria/Hysteria2/TUIC links that passed the UDP transport screen.", "- `globalping-city-diagnostic.json` — live Globalping Russian-city inventory + city probes.", "- `check-host-russia-nodes.json` — live Check-Host Russian node inventory.", "");
  await fs.writeFile(path.join(OUT_DIR, "results.md"), `${md.join("\n")}\n`, "utf8");
  await fs.writeFile(path.join(OUT_DIR, "results.json"), `${JSON.stringify({ generatedAt:new Date().toISOString(), scope:SCOPE, coreNodes:CORE_NODES, reports, globalping, checkHostDiscovery, inputSha256:hash(await fs.readFile(INPUT_FILE)) }, null, 2)}\n`, "utf8");
  console.log("RUSSIA CHECKER V2 COMPLETE");
}

main().catch(error => { console.error(`RUSSIA CHECKER V2 FAILED: ${error?.stack || error?.message || String(error)}`); process.exitCode = 1; });
